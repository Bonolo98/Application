//ets hash the password  initialise bcrypt===============================
...
exports.register = async (req, res) => {
  ...
  try {
    const data = await Pool.query(`SELECT * FROM users WHERE email= $1;`, [email]); //Check if user exist
    const arr = data.rows;
    if (arr.length != 0) {
    ...
    //take that hashing you initialised and use it to hash your password======================================
    else {
